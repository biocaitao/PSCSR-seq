...
cells <- ScaleData(object = cells,  vars.to.regress = c("nCount_RNA"))
...
data<-GetAssayData(object = cells, slot = "data")
dim(data[VariableFeatures(cells),])
library(pheatmap)
library(RColorBrewer)
m<-as.matrix(data[VariableFeatures(cells),])
pheatmap(m ,clustering_method = "ward.D2", show_colnames = F, fontsize_row = 1)
